...
router.get('/state', (req, res) => {
  ...
  // Playlist patches (v0.5.36+): when cooldown_suppress_fpp_playlist is
  // enabled, sequences in cooldown are disabled in FPP's playlist JSON so
  // FPP skips them in normal rotation. Off by default -- opt-in only.
  // Requires plugin v0.13.40+. Same logic as ShowPilot main v0.33.152.
...
// (5 min), or never handed off at all but too old to serve (2 hours).
setInterval(() => {
...
router.post('/playing', (req, res) => {
  ...
  if (name) {
    ...
    // Sequence-based PSA counter (v0.4.0+). Increment on every NON-PSA
...
router.post('/heartbeat', (req, res) => {
  ...
  // Reject public IPs as fppHost. We store this value to display in admin
  // status and surface in cover-art lookups — if an attacker had a
  // compromised showToken AND could spoof their source IP, an unvalidated
  // value here could be used as a confused-deputy upstream. FPP only ever
