...
const { progressBarConfig } = require('./progress-bar');
...
// {NOW_PLAYING_PROGRESS} (v0.33.206+): fraction of the current song played,
// 0..1, for the bar's first paint (rf-compat takes over after load).
// null when nothing is playing or the duration is unknown.
function computeInitialProgress(startedAtIso, durationSeconds) {
  if (!startedAtIso || !durationSeconds) return null;
  const startMs = Date.parse(startedAtIso);
  if (!isFinite(startMs)) return null;
  const f = (Date.now() - startMs) / 1000 / durationSeconds;
...
// Category header rows. `sequences` arrives pre-grouped by
// lib/categories.js#applyCategoryView; we just emit a header each time the
// category changes. No categorized sequences (or headers turned off) →
// output is byte-identical to the pre-category markup, so existing
// templates are untouched until an operator actually uses categories.
// MIRRORED in public/rf-compat.js#withCategoryHeaders — change both.
function withCategoryHeaders(sequences, cfg, rowFn) {
  const on = !cfg || cfg.viewer_show_categories !== 0;
  const anyCat = on && sequences.some(s => s.category && String(s.category).trim());
  if (!anyCat) return sequences.map(rowFn);
  const other = (cfg && typeof cfg.uncategorized_label === 'string' && cfg.uncategorized_label.trim()) || 'Other';
  const out = [];
  let prevKey = null;
...
function renderRaceGrid(sequences, tapCounts, cfg) {
  ...
  const rows = withCategoryHeaders(sequences, cfg, seq => {
...
function renderTemplate(template, state) {
  ...
  // {NOW_PLAYING_PROGRESS} (v0.33.206+) — a progress bar plus time left,
  // placed wherever the template author wants it. ShowPilot extension (not
  // an RF placeholder). Styled by rf-compat's injected .sp-progress CSS,
  // which templates can override; rf-compat ticks it every second. Hidden
  // (sp-progress--idle) when nothing is playing or duration is unknown.
  if (html.indexOf('{NOW_PLAYING_PROGRESS}') !== -1) {
    const frac = computeInitialProgress(state.nowPlayingStartedAtIso, state.nowPlayingDurationSeconds);
    const idle = frac === null ? ' sp-progress--idle' : '';
    const pct = frac === null ? 0 : Math.round(frac * 1000) / 10;
    html = html.split('{NOW_PLAYING_PROGRESS}').join(
      `<div class="sp-progress sp-progress--inline${idle}" data-showpilot-progress role="progressbar" aria-label="Song progress" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${Math.round(pct)}">` +
        `<div class="sp-progress-track"><div class="sp-progress-fill" style="width: ${pct}%"></div></div>` +
        `<span class="sp-progress-time" data-showpilot-progress-time>${initialTimerText}</span>` +
      `</div>`
    );
  }
...
.jukebox-list > div, .cell-vote-playlist > div { font-size: inherit; }
/* Category header rows. Injected at the TOP of <head> so any
   template rule for .sequence-category-header wins the cascade. flex-basis +
   grid-column make the header span the full row in flex AND grid list
   wrappers (.jukebox_table / .voting_table vary by template). */
.sequence-category-header { flex: 0 0 100%; width: 100%; grid-column: 1 / -1; box-sizing: border-box; margin: 0.9em 0 0.35em; padding: 0.3em 0.2em; font-weight: 700; font-size: 1.05em; letter-spacing: 0.06em; text-transform: uppercase; text-align: left; border-bottom: 1px solid currentColor; opacity: 0.9; }
.sequence-category-header:first-child { margin-top: 0.2em; }
</style>
`;
  if (html.includes('<head>')) {
  ...
  const bootstrap = {
    ...
    serverNowMs: Date.now(), // rough first clock estimate; refined by /api/state polls
...
.race-instructions p { margin: 0; }
  ...
  <script src="/rf-compat.js?v=79"></script>
  `;
  ...
  // ---- Demo banner (v0.31.0+) ----
  // Demo banner — only injected when demoMode is explicitly enabled.
  // Production installs skip this entirely, keeping view-source clean.
