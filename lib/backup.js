...
// Table/column names below come from this file's own hardcoded SECTIONS
// list, from SQLite's own PRAGMA table_info() output, or (filtered through
// one of those two) from an uploaded backup's row keys — never directly
// from request/backup data otherwise. See lib/sql-ident.js for why every
// one of them is still checked with assertIdent() before being spliced
// into a SQL string built outside of .prepare()'s/.exec()'s argument.
function tableColumns(tableName) {
  assertIdent(tableName);
  const db = getDb();
  const sql = 'PRAGMA table_info(' + tableName + ')';
  return db.prepare(sql).all().map(c => c.name);
}
...
function buildBackup(options) {
  ...
  for (const section of SECTIONS) {
    assertIdent(section.table);
    const sql = 'SELECT * FROM ' + section.table;
...
function inspectBackup(backup) {
  ...
  for (const section of SECTIONS) {
    ...
    const countSql = 'SELECT COUNT(*) AS n FROM ' + section.table;
    const destCount = db.prepare(countSql).get().n;
    ...
    if (section.naturalKey && backupRows.length > 0) {
      assertIdent(section.naturalKey);
      const backupKeys = new Set(backupRows.map(r => String(r[section.naturalKey])));
      const destRowsSql = 'SELECT ' + section.naturalKey + ' FROM ' + section.table;
      const destRows = db.prepare(destRowsSql).all();
    ...
    for (const section of SECTIONS) {
      ...
      if (policy === 'replace') {
        ...
        if (section.table === 'config') {
          ...
          if (filteredRows.length > 0) {
            ...
            if (updateCols.length > 0) {
              updateCols.forEach(assertIdent);
          ...
          assertIdent(section.table);
          const deleteSql = 'DELETE FROM ' + section.table;
          ...
          if (filteredRows.length > 0) {
            ...
            for (const row of filteredRows) {
              ...
              const sql = 'INSERT INTO ' + section.table + ' (' + cols.join(', ') + ') VALUES (' + placeholders + ')';
              db.prepare(sql).run(row);
            }
          }
        }
        ...
        for (const row of filteredRows) {
          ...
          const existingSql = 'SELECT id FROM ' + section.table + ' WHERE ' + section.naturalKey + ' = ? LIMIT 1';
          const existing = db.prepare(existingSql).get(naturalKeyValue);

          if (existing) {
            // UPDATE — drop the primary key from the SET clause to
            // preserve the destination's id (foreign key safety). The
            // destination row to update is identified by its OWN id
            // (existing.id, from the lookup above) via a bind parameter,
            // not the backup row's id, which is why it's bound separately
            // rather than reused from `row`.
            const updateCols = Object.keys(row).filter(c => c !== section.primaryKey);
            if (updateCols.length > 0) {
              updateCols.forEach(assertIdent);
              assertIdent(section.primaryKey);
              const setClause = updateCols.map(c => `${c} = @${c}`).join(', ');
              const sql = 'UPDATE ' + section.table + ' SET ' + setClause + ' WHERE ' + section.primaryKey + ' = @__existingId';
              db.prepare(sql).run({ ...row, __existingId: existing.id });
            }
          } else {
