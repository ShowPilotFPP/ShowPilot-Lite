...
function renderTemplate(template, state) {
  ...
  // ---- Normalize document structure (v0.5.22) ----
  ...
  // every subsequent injection can target `<head>` and trust it exists.
  {
    ...
    // Capture and remove the outermost <html ...> opening tag and (if any)
    // closing </html>.
    html = html.replace(/<html\b[^>]*>/i, '');
    html = html.replace(/<\/html\s*>\s*$/i, '');

    // Extract existing <head>...</head> if any. Tolerate attributes on the
    // head opening tag.
    let headInner = '';
    ...
    // Detect existing <body>. If absent (the common RF case), wrap the
    // remaining content in <body>...</body> so subsequent injections that
    // target <body> have an anchor and don't fall back to "prepend to
    // document" — which would land BEFORE the DOCTYPE and re-trigger
    // quirks mode.
  ...
  // ---- Inject viewport meta if the template doesn't have one (v0.5.23) ----
  ...
  // The other 5 (and most community templates derived from them) lack one.
  // RF's hosted page shell injects a viewport meta for free; we have to do
  // it ourselves.
  //
  // Conservative: only inject when the template HAS NOT declared one.
  ...
  // ---- Inject RF-compat CSS reset (v0.5.21) ----
  ...
  //      overrides reaching the visible text. Our v0.5.13 inner-wrapper
  ...
  //
  // Both are fixed by a tiny low-specificity reset:
...
`;
  ...
  const bootstrap = {
    ...
    raceEndsAt: cfg.race_ends_at || null,
    raceWinner: cfg.race_winner || null,
    raceTargetTaps: cfg.race_target_taps || 0,
