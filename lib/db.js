...
const { assertIdent } = require('./sql-ident');
...
CREATE TABLE IF NOT EXISTS config (
  ...
  -- Vote shifting (v0.5.6+): when prevent_multiple_votes=1 AND this is on,
  ...
  -- psa_trigger_mode (v0.4.0+): how psa_frequency is counted.
...
CREATE TABLE IF NOT EXISTS sequences (
  ...
  -- cooldown_minutes (v0.3.2+): when this sequence starts playing, suppress
...
function columnExists(table, column) {
  ...
  const info = db.prepare(sql).all();
  return info.some(c => c.name === column);
}

// Add a column from one of this file's hardcoded migration lists — `spec`
// is a trusted SQL fragment (type + DEFAULT clause) straight from the
// migration entry, never request data; `table`/`column` still go through
// assertIdent() since they're the actual identifiers being spliced in.
function addColumn(table, column, spec) {
  assertIdent(table);
  assertIdent(column);
  const sql = 'ALTER TABLE ' + table + ' ADD COLUMN ' + column + ' ' + spec;
  db.exec(sql);
}
...
const configMigrations = [
  ...
  // v0.4.0+: choose how psa_frequency is counted. See schema comment
  // above for semantics. Existing installs default to 'interactions' so
  // upgrade behavior is identical to v0.3.x.
  ['psa_trigger_mode', `TEXT DEFAULT 'interactions'`],
  ['sequences_since_last_psa', 'INTEGER DEFAULT 0'],
  ['check_viewer_present', 'INTEGER DEFAULT 0'],
  ...
  // cooldown_suppress_fpp_playlist (v0.5.36+): when 1, sequences in cooldown
  ...
  ['plugin_version', 'TEXT'],
  ['plugin_fpp_host', 'TEXT'],   // IP/hostname of FPP — used for plugin status display and cover-art proxy
  // ---- Now-playing player bar (kept in Lite as a display-only widget) ----
  // Lite has no audio, but the bar itself is still shown to viewers as a
  // "now playing" indicator: cover art + sequence title. Decoration settings
  // control the visual styling of that bar.
  ['player_decoration', `TEXT DEFAULT 'none'`],     // viewer player decoration theme
  ['player_decoration_animated', 'INTEGER DEFAULT 1'],
  ['page_snow_enabled', 'INTEGER DEFAULT 0'],       // legacy boolean; superseded by page_effect (v0.5.0+). Kept for backward-compat reads.
  // Page-wide ambient effects (v0.5.0+). See ShowPilot main's db.js for the
  // full rationale. Values: 'none' | 'snow' | 'leaves' | 'fireworks' | 'hearts' |
  // 'stars' | 'bats' | 'confetti' | 'petals' | 'embers' | 'bubbles' | 'rain'.
  ...
  // viewer_show_player_bar: master switch for the now-playing bar in Lite.
  // When 0, the bar is hidden entirely on viewer pages — for admins who
  // want a pure voting/jukebox UI with no song display.
  ['viewer_show_player_bar', 'INTEGER DEFAULT 1'],
  ...
  // Location code (v0.5.26+): when enabled, viewers must enter a code
  ...
  // viewer_url (Lite-only)
  ['viewer_url', `TEXT DEFAULT ''`],
  // Race mode (v0.33.155+)
  ['race_duration_seconds',   'INTEGER DEFAULT 60'],
  ['race_end_on_sequence_end','INTEGER DEFAULT 0'],
  ['race_target_taps',        'INTEGER DEFAULT 0'],
  ['race_interrupt_winner',   'INTEGER DEFAULT 0'],
  // Race viewer customisation (v0.5.42+)
  ['race_instructions_text',  'TEXT'],
  ['race_font_size',          "TEXT DEFAULT '0.9em'"],
  ['race_use_template_css',   'INTEGER DEFAULT 0'],
  ['race_active',             'INTEGER DEFAULT 0'],
  ['race_started_at',         'TEXT'],
  ['race_ends_at',            'TEXT'],
  ['race_winner',             'TEXT'],
  // Translation settings (v0.5.45+)
  ['translation_enabled',     'INTEGER DEFAULT 0'],
  ['translation_backend',     "TEXT DEFAULT 'mymemory'"],
  ['translation_api_url',     "TEXT DEFAULT 'https://libretranslate.com'"],
  ['translation_api_key',     'TEXT'],
  // Sequence categories (see lib/categories.js)
  ['sequence_categories',     "TEXT DEFAULT '[]'"],     // JSON [{name, enabled}] in display order
  ['viewer_show_categories',  'INTEGER DEFAULT 1'],     // 1 = group viewer list by category with header rows
  ['uncategorized_label',     "TEXT DEFAULT 'Other'"],  // header text for sequences with no category
  // Song progress bar on the viewer page (lib/progress-bar.js; ShowPilot v0.33.206)
  ['viewer_progress_bar',           'INTEGER DEFAULT 0'],     // 1 = show a progress bar on every viewer page
  ['viewer_progress_bar_position',  "TEXT DEFAULT 'player'"], // 'player' | 'screen-top' (0.5.55's 'top'/'bottom' read as 'player')
  ['viewer_progress_bar_show_time', 'INTEGER DEFAULT 1'],     // 1 = show time left next to the bar
  ['viewer_progress_bar_color',     "TEXT DEFAULT ''"],       // CSS color; '' = built-in default
];
for (const [col, spec] of configMigrations) {
  if (!columnExists('config', col)) {
    addColumn('config', col, spec);
  }
}

// One-shot migration (v0.5.0+): legacy page_snow_enabled=1 installs were
// upgraded to add the new page_effect column with default 'none'. Promote
// the boolean so existing snowing instances stay snowing. See ShowPilot
// main's db.js for the full rationale.
try {
...
} catch (e) { /* best-effort migration; defaults are sensible */ }
...
const sequenceMigrations = [
  ...
  ['plays_since_hidden', 'INTEGER DEFAULT 0'],
  ['display_order', 'INTEGER DEFAULT 0'],
  // cooldown_minutes (v0.3.2+): per-sequence "after I play, suppress me
...
// Admin layout preference (v0.33.208+): NULL/'new' = the redesigned admin
// (/admin/new.html, the default), 'classic' = the original single page.
// layout_notice_seen: the one-time "you can switch back" notice was dismissed.
if (!columnExists('users', 'admin_layout')) {
  db.exec(`ALTER TABLE users ADD COLUMN admin_layout TEXT`);
}
if (!columnExists('users', 'layout_notice_seen')) {
  db.exec(`ALTER TABLE users ADD COLUMN layout_notice_seen INTEGER DEFAULT 0`);
}
// Cockpit tile layout (main v0.33.214 / Lite v0.5.61): JSON array of
// { id, size } per user; NULL = the built-in default layout.
if (!columnExists('users', 'cockpit_layout')) {
  db.exec(`ALTER TABLE users ADD COLUMN cockpit_layout TEXT`);
}
...
const templateMigrations = [
  ...
  // (v0.5.3+) See ShowPilot main's db.js for the full rationale of this
  // column. Tracks the SHA-256 of the file content the seeder last wrote,
  // letting us detect "admin edited the row" so file-driven refreshes
  // don't clobber custom changes.
  ['builtin_source_hash', `TEXT DEFAULT ''`],
];
for (const [col, spec] of templateMigrations) {
  if (!columnExists('viewer_page_templates', col)) {
    addColumn('viewer_page_templates', col, spec);
  }
}
...
// One-shot rename cleanup (main v0.33.210 / Lite v0.5.58): the default
// template is copied from public/viewer.html into the DB once, at first
// boot, and never refreshed — so shows set up before the OpenFalcon ->
// ShowPilot rename still had "Powered by OpenFalcon" in its footer and the
// name "Default (OpenFalcon)". Fix built-in rows only (never user-created
// templates), and only the exact old text, so it's a no-op once applied.
{
  const OLD = 'Powered by OpenFalcon';
  const NEW = 'Powered by ShowPilot';
  const rows = db.prepare(`SELECT id, name, html, draft_html FROM viewer_page_templates WHERE is_builtin = 1`).all();
  const nameTaken = !!db.prepare(`SELECT 1 FROM viewer_page_templates WHERE name = 'Default (ShowPilot)'`).get();
  for (const row of rows) {
    const html = row.html || '';
    const draft = row.draft_html || '';
    const rename = row.name === 'Default (OpenFalcon)' && !nameTaken;
    if (!html.includes(OLD) && !draft.includes(OLD) && !rename) continue;
    db.prepare(`UPDATE viewer_page_templates SET html = ?, draft_html = ?, name = ? WHERE id = ?`).run(
      html.split(OLD).join(NEW), draft.split(OLD).join(NEW),
      rename ? 'Default (ShowPilot)' : row.name, row.id);
    console.log(`[migration] Updated old OpenFalcon branding in template "${row.name}"`);
  }
}

// Seed built-in viewer templates (v0.5.0+, refresh logic v0.5.3+)
// See ShowPilot main's db.js for the full rationale — this is a
// code-for-code parity port (non-audio change, both repos).
{
  const fs = require('fs');
  const path = require('path');
  ...
  for (const file of files) {
    ...
    if (existing.is_builtin !== 1) continue;
    ...
    console.log(`[ShowPilot-Lite] Refreshed built-in template '${name}' from disk (file changed)`);
  }
}
...
function updateConfig(updates) {
  ...
  const sql = 'UPDATE config SET ' + setClause + ' WHERE id = 1';
...
function setNowPlaying(sequenceName, secondsPlayed) {
  ...
  //
  // By anchoring once and trusting it, the viewer's progress display and our
  // anchor stay in fixed relationship — the displayed elapsed time advances
  // smoothly off a stable started_at instead of jittering as new reports
  // come in.
  //
...
function getNextUp(cfg, nowPlayingName) {
  // Nothing playing (FPP idle/stopped): show no "Up Next" rather than a guess.
  // The plugin doesn't queue anything while FPP is idle, so no tier below is
  // actually what will play next.
  if (!nowPlayingName) return null;
  ...
  // Skip disabled categories: songs synced from other playlists (e.g. last
  // season's) stay visible=1 and reuse the same sort_order numbers, so without
  // this the fallback can name a song the viewer page doesn't even list.
  // Lazy require: lib/categories requires this module.
  const { isCategoryDisabled } = require('./categories');
  const candidates = currentSortOrder !== null
    ? db.prepare(`
        SELECT name, category FROM sequences
        WHERE sort_order > ? AND visible = 1 AND is_psa = 0
        ORDER BY sort_order ASC
      `).all(currentSortOrder)
    : db.prepare(`
        SELECT name, category FROM sequences
        WHERE visible = 1 AND is_psa = 0
        ORDER BY sort_order ASC
      `).all();
  const nextRow = candidates.find(r => !isCategoryDisabled(r.category, cfg));
...
function createSnapshot(name, description) {
  ...
  const tx = db.transaction((snapName, snapDesc) => {
    const seqs = db.prepare(`
      SELECT name, display_name, artist, category, image_url,
    ...
    const insertItem = db.prepare(`
      INSERT INTO sequence_snapshot_items
        (snapshot_id, name, display_name, artist, category, image_url,
         duration_seconds, visible, votable, jukeboxable, is_psa, sort_order, display_order)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
    for (const s of seqs) {
      insertItem.run(
        snapshotId, s.name, s.display_name, s.artist, s.category, s.image_url,
...
function restoreSnapshot(snapshotId) {
  const tx = db.transaction((id) => {
    ...
    const updateExisting = db.prepare(`
      UPDATE sequences
      SET display_name = ?, artist = ?, category = ?, image_url = ?,
          duration_seconds = ?, visible = ?, votable = ?, jukeboxable = ?, is_psa = ?,
          sort_order = ?, display_order = ?
      WHERE name = ?
    `);
    const insertNew = db.prepare(`
      INSERT INTO sequences
        (name, display_name, artist, category, image_url,
         duration_seconds, visible, votable, jukeboxable, is_psa, sort_order, display_order)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
    ...
    for (const it of items) {
      ...
      if (existing) {
        updateExisting.run(
          it.display_name, it.artist, it.category, it.image_url,
          it.duration_seconds, it.visible, it.votable, it.jukeboxable, it.is_psa,
          it.sort_order, it.display_order, it.name
        );
      } else {
        insertNew.run(
          it.name, it.display_name, it.artist, it.category, it.image_url,
          it.duration_seconds, it.visible, it.votable, it.jukeboxable, it.is_psa,
...
function setUserAdminLayout(id, layout) {
  db.prepare(`UPDATE users SET admin_layout = ? WHERE id = ?`).run(layout, id);
}

function setUserCockpitLayout(id, layoutJson) {
  db.prepare(`UPDATE users SET cockpit_layout = ? WHERE id = ?`).run(layoutJson, id);
}

function setUserLayoutNoticeSeen(id) {
  db.prepare(`UPDATE users SET layout_notice_seen = 1 WHERE id = ?`).run(id);
}
...
module.exports = {
  ...
  createUser,
  updateUser,
  setUserTheme, setUserAdminLayout, setUserLayoutNoticeSeen, setUserCockpitLayout,
  setUserPassword,
  deleteUser,
  recordUserLogin,
  countUsers,
