...
<div class="of-page-wrap">
  ...
  <!-- Now Playing — always visible (v0.5.8+).
...
<div style="text-align:center; padding: 2rem 1rem 1rem;">
  ...
  text: {
    ...
    // Prop type hints (v0.5.10+). Used by the admin block editor to render
...
<div {after-hours-message}>
  ...
  nowPlaying: {
    ...
    render(p) {
      // v0.5.8+: this block is no longer wrapped in {jukebox-dynamic-container}.
...
<div>
  ...
  nextUp: {
    ...
    defaultProps: { titleColor: '#dc2626' },
    render(p) {
      // v0.5.8+: standalone block for the upcoming song. Always visible.
...
function listBlockTypes() {
  return Object.entries(BLOCK_TYPES).map(([type, def]) => ({
    ...
    // propTypes (v0.5.10+) — optional per-prop control hints used by
